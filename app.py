...
/* NEVER change .CodeMirror-scroll's vertical padding: its 50px padding-bottom
   is balanced against a -50px margin-bottom so getScrollInfo().clientHeight
   equals the visible height. Overriding it (we had 100px) inflates the height
   CodeMirror thinks it has, so scrolling down leaves the caret hidden below
   the real fold while CodeMirror believes it is still visible. Breathing room
   above/below the text goes on .CodeMirror-lines, which CodeMirror accounts
   for in its scroll math. */
#sourceWrap .CodeMirror-scroll{padding-left:42px;padding-right:42px}
#sourceWrap .CodeMirror-lines{max-width:900px;margin:0 auto;padding:24px 0 48px}
...
function registerVimLeaderCommands(){
  ...
  const defs={
    ...
    nnSmartInsert:cm=>{try{CodeMirror.Vim.handleKey(cm,'a')}catch(_){}},
...
function dlgSetMode(m,target){
  ...
  if(m==='insert'){
    ...
    if(t){try{t.focus();const v=t.value||'';t.setSelectionRange(v.length,v.length)}catch(_){t&&t.focus&&t.focus()}}
  }else{
    const a=document.activeElement;if(a&&a.blur&&dlg.contains(a))a.blur();
...
function dlgConfirm(){
  const dlg=activeVimDialog();if(!dlg)return;
  if(dlg.id==='newDialog'){
    if(!$('newTitle').value.trim()){dlgSetMode('insert',$('newTitle'));status('タイトルを入力してください');return}
    $('newForm').requestSubmit();
  }else if(dlg.id==='edgeDialog'){
...
window.addEventListener('keydown',e=>{
  ...
  if(dlgMode==='insert'){
    ...
    if(e.key==='Enter'){
      const a=document.activeElement;
      if(dlg.id==='edgeDialog'&&a&&a.id==='edgeNewTitle'){e.preventDefault();e.stopPropagation();$('edgeNewBtn').click();return}
      e.preventDefault();e.stopPropagation();dlgSetMode('normal');dlgConfirm();return;
    }
    return;
  }
